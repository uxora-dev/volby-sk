...
export class RegionMapComponent {
  ...
  protected readonly okresPaths = this.maps.paths();
  protected readonly data = computed(() => this.maps.mapFor(this.electionId())());

  protected readonly selected = signal<string | null>(null);

  /** Okresy s predpočítanou cestou a farbou víťaza. */
  protected readonly okresy = computed(() => {
    const paths = this.okresPaths();
    const d = this.data();
    if (!paths || !d) return [];
    return paths.okresy.map((o) => {
      const res = d.okresy[o.code];
      const fill = res ? partyColor({ abbr: res.w, name: res.w }) ?? NEUTRAL : NODATA;
      return { code: o.code, d: o.d, fill };
    });
  });

  /** Detail vybraného okresu — názov + top strany s farbami. */
  protected readonly selectedInfo = computed(() => {
    const code = this.selected();
    const d = this.data();
    const paths = this.okresPaths();
    if (!code || !d || !paths) return null;
    const res = d.okresy[code];
    if (!res) return null;
    const name = paths.okresy.find((o) => o.code === code)?.name ?? code;
    return {
      name,
  ...
  /** Legenda — víťazné strany a v koľkých okresoch vyhrali. */
  protected readonly legend = computed(() => {
    const d = this.data();
    if (!d) return [];
    const counts = new Map<string, number>();
    for (const o of Object.values(d.okresy)) counts.set(o.w, (counts.get(o.w) ?? 0) + 1);
